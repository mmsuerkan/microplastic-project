...
# =============================================
# OTOMATİK PARÇACIK TAKİP SİSTEMİ V2
# Değişiklik: Silindir bölgesi %10-%90 (önceki: %25-%75)
# Elle ROI seçimi YOK - Tam otomatik
# =============================================
...
def auto_track_particle(video_path, output_dir="output_results", show_video=True, debug=False):
    ...
    # =============================================
    # PARAMETRELER (V2 - Genişletilmiş arama bölgesi)
    # =============================================
    # Silindir bolgesi - V2'de genisletildi
    CYLINDER_LEFT = int(width * 0.10)   # V1: 0.25 -> V2: 0.10
    CYLINDER_RIGHT = int(width * 0.90)  # V1: 0.75 -> V2: 0.90
    ...
    # =============================================
    # ARKA PLAN MODELI OLUSTUR
    # =============================================
    print("Arka plan modeli olusturuluyor...")

    # Ilk birkac frame'i oku ve ortalama al (arka plan)
    bg_frames = []
    ...
    # =============================================
    # PARCACIK TESPITI - Ilk hareket eden nesneyi bul
    # =============================================
    print("Parcacik araniyor...")
        ...
        for cnt in contours:
            ...
            if MIN_BLOB_AREA <= area <= MAX_BLOB_AREA:
                ...
                if M["m00"] > 0:
                    ...
                    # Ust yarida mi? (parcacik yukaridan duser)
                    if cy < height * 0.4:
                        initial_pos = (cx, cy)
                        particle_found = True
                        detection_frame = frame_idx
                        print(f"Parcacik bulundu! Frame {frame_idx}, Konum: ({cx}, {cy})")
                        break
    ...
    while True:
        ...
        if show_video:
            ...
            cv2.imshow("Auto Tracking V2", display)
    ...
    # Piksel/metre orani (28.5 cm = 0.285 m mesafe varsayimi)
    # Bu deger kalibrasyon ile ayarlanmali
    KNOWN_DISTANCE_M = 0.285
    KNOWN_DISTANCE_PX = vertical_pixels  # Varsayim: tum dusus bu mesafe
    ...
    with open(summary_path, 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writerow(['Tracker Version', 'V2', ''])
        ...
        writer.writerow(['Ortalama dX', f'{np.mean([v[2] for v in vector_data]):.4f}', 'px/frame'])
        writer.writerow(['Ortalama dY', f'{mean_dy:.4f}', 'px/frame'])
        writer.writerow(['Tahmini Hiz', f'{speed_mps:.4f}', 'm/s'])
        writer.writerow(['Tahmini Hiz', f'{speed_mps*100:.2f}', 'cm/s'])
    ...
    if vector_data and len(vector_data) > 5:
        ...
        # 1. Vektor alani (ok grafigi)
        ax1 = axes[0, 0]
        vector_arr = np.array(vector_data)
        # Her 3 noktadan birini goster (cok yogun olmasin)
        ...
        # 2. dX ve dY zaman serisi (m/s cinsinden)
        ax2 = axes[0, 1]
        frames = np.arange(len(vector_arr))
        # Piksel -> m/s donusumu: (piksel/frame) * (fps) * (metre/piksel)
        # 28.5 cm = 0.285 m mesafe, dikey hareket piksel cinsinden
        ...
        # 5. FFT - Yatay hareket (dX) frekansi
        ax5 = axes[0, 2]
        N = len(vector_arr)
        T = 1.0 / fps  # Ornekleme periyodu
        dx_values = vector_arr[:, 2]
        dx_fft = np.fft.fft(dx_values - np.mean(dx_values))  # DC bileseni cikar
        freqs = np.fft.fftfreq(N, T)[:N//2]
        dx_power = 2.0/N * np.abs(dx_fft[0:N//2])

        ax5.plot(freqs, dx_power, 'b-', alpha=0.7)
        # Dominant frekans
        dominant_idx = np.argmax(dx_power[1:]) + 1  # DC haric
        ...
        ax5.set_xlim(0, fps/4)  # Nyquist/2 ye kadar goster

        # 6. FFT - Dikey hareket (dY) frekansi
        ax6 = axes[1, 2]
